import { getUrlData, getCookie } from "../js/module/globalSettings.js";
...
const handleDatePicker = (filterDoms) => {
  const { date, form } = filterDoms;
  const { calender, startDate, endDate } = date;

  calender.addEventListener('click', async () => {
    try {
      await startDate.showPicker();
      let timeEl = form.querySelector("input[name='time']:checked");
  ...
  tagBtnEls.forEach((tagBtnEl) => {
    tagBtnEl.addEventListener('click', async (e) => {
      ...
      if (tagName == "source" || tagName == "severity") {
        filterData["body"][tagName] = dealingList.filter((item) => { item != tagName })
      }
...
const handleChangeTag = async (data) => {
  ...
  // source 
  if (data["source"].length != 0) [...sources].filter(item => {
    // console.log("source");
    item.checked = data["source"].includes(item.value.toLowerCase()) ? true : false
  })
  // severity - 將critical/major/minor box打勾
  if (data["severity"].length != 0) {
    console.log("severity");
    let severityList = [...severitys]
    severityList[0].checked = data["severity"].includes("critical") ? true : false
    severityList[1].checked = data["severity"].includes("major") ? true : false
    severityList[2].checked = data["severity"].includes("minor") ? true : false
  }
  // acknowledge - 將check/uncheck box打勾
  if (data["acknowledge"].length != 0) {
    console.log("acknowledge");
    let acknowledgeList = [...acknowledges]
    acknowledgeList[0].checked = data["acknowledge"].includes("true") ? true : false
    acknowledgeList[1].checked = data["acknowledge"].includes("false") ? true : false
  }
  // solved - 將check/uncheck box打勾
  if (data["solved"].length != 0) {
    console.log("solved");
    let solvedList = [...solveds]
    solvedList[0].checked = data["solved"].includes("true") ? true : false
    solvedList[1].checked = data["solved"].includes("false") ? true : false
  }

  // filter date
  if (data["endDate"] == defaultEnd) {
    switch (data["startDate"]) {
      case in1Day:
        dateIn1Day.click();
        break;
      case in1Week:
...
const handleApplyBtn = async () => {
  ...
  //跳轉頁面傳值
  let passData = {
    "pageNum": 1, //filter送出固定為1
    "alertPk": "",
    "startDate": startDate.value,
    "endDate": endDate.value,
    "idORDescription": idOrDescription[0].value, //輸入框
    "source": sourceList,
    "severity": severityList,
    "acknowledge": acknowledgeList,
    "solved": solvedList,
  }
...
const handleForm = async (filterDoms) => {
  ...
  //click radio to change calender
  // console.log("setting calender");
  for (let time of timeList) {
    time.addEventListener('click', async () => {
      endDate.value = defaultEnd
      switch (time.id) {
        case "dateIn1Day":
          startDate.value = in1Day
          break;
        case "dateIn1Week":
          startDate.value = in1Week
          break;
        case "dateIn1Month":
          startDate.value = in1Month
          break;
      }
    });
  }
...
// 根據ran、ru數量產生checkbox
const handleSources = async () => {
  let serverIp = await getCookie("serverIp")
  let sourceList = []
  sourceList = sourceList.concat(Object.keys(serverIp["RAN"]))
  sourceList = sourceList.concat(Object.keys(serverIp["RU"]))
  console.log(sourceList);
  for (let service of sourceList.values()) {
    sourceCheckboxesEl.innerHTML += `
    <div class="c-checkbox__set">
      <input type="checkbox" name="source" id="source${service}" class="form-check-input c-checkbox__input"
        value="${service}">
      <label for="source${service}" class="c-checkbox__label">${service}</label>
    </div>
    `
  }
}
...
const defaultAction = async () => {
  console.log(ranNotifyContentMap);
  console.log(ruNotifyContentMap);
  await handleSources()

  let passedData = getUrlData()
  alertPk = passedData?.alertPk == undefined ? "" : passedData.alertPk
